...
- 当前状态：`landscape_contract_revised_existing_portrait_assets_need_regeneration`
- 图片生成：旧 B00、B07、B06、B01、B02 批次曾生成 canonical PNG，但当前文件仍是旧竖屏或低于横屏目标的历史参考图；本轮仅完成 16:9 横屏宽屏合同修复。所有已存在 PNG 在横屏交付前必须按 `3840x2160`、`16:9`、无 alpha 的新合同重新生成并重新 QC。B07 具体色彩、材质、地域色板、势力符号、横屏宽屏构图和低魔光效数据已补入 `art/b07-style-reference-data.json`、资产索引、提示词和线程合同；未生成角色/地点/道具批次仍待单独派发。
- 产物日期：2026-06-06
...
全局资产提示词不是最终视频帧提示词。每条全局 prompt 都已经写入 literal `output_format`，并把同一格式要求合并进 `copy_ready`。机器字段使用 `art-room` schema 枚举值；16:9 横屏宽屏、3840x2160 px、透明派生层和线控层要求保留在 notes 字段和 copy-ready 文本中：

| 资产类型 | schema 交付类型 | 图片格式 | 画幅与最低分辨率 | 背景/透明策略 |
| --- | --- | --- | --- | --- |
| 角色/族群模板 | neutral_master_card（notes: neutral_master_character_card） | png | project_defined（16:9 landscape widescreen），3840x2160 | neutral_plain / forbidden；透明 cutout 必须另建 PNG/SVG |
| 地点主场景 | location_scene_card | png | project_defined（16:9 landscape widescreen），3840x2160 | scene_context / forbidden |
| 道具/符号 | neutral_master_card（notes: neutral_master_prop_card） | png | project_defined（16:9 landscape widescreen），3840x2160 | neutral_plain / forbidden；精确符号另建线稿/透明层 |
| 三势力徽章旗帜根资产 | neutral_master_card（notes: faction_emblem_flag_master_prop_card） | png | project_defined（16:9 landscape widescreen），3840x2160 | neutral_plain / forbidden；精确徽章、旗面 mask、SVG/PNG 线控层另建 |
| 服装系统 | style_reference（notes: neutral_costume_system_card） | png | project_defined（16:9 landscape widescreen），3840x2160 | neutral_plain / forbidden；服装 cutout 另行规划 |
| 风格参考 | style_reference（notes: style_reference_board） | png | project_defined（16:9 landscape widescreen），3840x2160 | neutral_plain / forbidden |
...
5. `art/series-thread-plan.json` 已拆分为三势力徽章旗帜基础母版、风格、服装、角色、地点、道具/符号的 8 个依赖批次；所有批次的当前目标已改为 16:9 横屏宽屏重生与 QC。
...
11. B06_COSTUMES 的旧 canonical PNG 仍可作为服装结构参考，但不是横屏可交付文件；K001-K006 必须按 3840x2160、16:9、无 alpha 的新合同重新生成并重新 QC。旧反馈中关于 K004、K005、K006 的符号与服装结构要求继续保留为重生约束。
...
15. B01_CORE_CHARACTERS 的旧 canonical PNG 仍可作为身份和反馈参考，但不是横屏可交付文件；C001-C007、C023-C026 必须按横屏合同重新生成。C001 的脸型发型硬锁、C023-C026 的群众层级区分继续保留为重生约束。
16. B02_NORTHERN_AND_FACTION_TEMPLATES 的旧 canonical PNG 仍可作为身份和反馈参考，但不是横屏可交付文件；C017-C021 必须按横屏合同重新生成。C017-C019 的虫族帝国徽章、奴兵/虫兵形态和 P017 派生规则继续保留为重生约束。

## 生成批次计划

预设批次写入 `art/series-thread-plan.json`，当前横屏合同已统一修复；现有旧竖屏图片只作历史参考，所有已有或未生成批次都需要横屏生成或横屏 QC：

|  顺序 | 批次                                 | 内容                                  | 依赖                                                                                        | 状态                                                    |
| --: | ---------------------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------- |
|   1 | B00_FACTION_EMBLEM_FLAGS           | 三势力徽章旗帜基础母版                         | 无                                                                                         | needs_landscape_regeneration_qc                       |
|   2 | B07_STYLE                          | 全剧风格板                               | B00_FACTION_EMBLEM_FLAGS                                                                  | needs_landscape_regeneration_qc                       |
|   3 | B06_COSTUMES                       | 服装系统                                | B00_FACTION_EMBLEM_FLAGS、B07_STYLE                                                        | needs_landscape_regeneration_qc                       |
|   4 | B01_CORE_CHARACTERS                | 核心、追捕线、沈家旧档、普通平民、逃难流民、边墙军户与墙下集市群像角色 | B00_FACTION_EMBLEM_FLAGS、B07_STYLE、B06_COSTUMES                                           | needs_landscape_regeneration_qc                       |
|   5 | B02_NORTHERN_AND_FACTION_TEMPLATES | 北境具名角色与族群/肃明层级模板                    | B00_FACTION_EMBLEM_FLAGS、B07_STYLE、B06_COSTUMES                                           | needs_landscape_regeneration_qc                       |
...
|   8 | B05_PROPS_SYMBOLS                  | 核心道具与精确符号                           | B00_FACTION_EMBLEM_FLAGS、B07_STYLE、B01_CORE_CHARACTERS、B02_NORTHERN_AND_FACTION_TEMPLATES | needs_landscape_generation                            |
