...
- 图片生成：B00_FACTION_EMBLEM_FLAGS、B07_STYLE 与 B06_COSTUMES 已完成并通过 QC；B00 线程 019e9626-3a1e-7a02-95df-e63b532fc205 已生成 P016-P018，B07 线程 019e963e-3a20-7d33-b16d-07df70d6f027 已生成 F001-F006，B06 在当前线程使用内置 imagegen 生成 K001-K006，并在本轮重新生成 K004-K006 后写入 canonical 路径。B01_CORE_CHARACTERS 已完成 C001-C007、C023-C026；C001 按用户提供的沈维桑旧参考板硬锁脸型与发型，C023-C026 已按反馈重新拉开流民、军户、平民和墙下集市群像服饰系统。B02_NORTHERN_AND_FACTION_TEMPLATES 已完成 C017-C021；C017-C019 已按反馈统一虫族帝国徽章并修正奴兵/虫兵形态。B07 具体色彩、材质、地域色板、势力符号、竖屏构图和低魔光效数据已补入 `art/b07-style-reference-data.json`、资产索引、提示词和线程合同；其余角色/地点/道具批次仍待单独派发。
- 产物日期：2026-06-06
...
| F005 | 9:16 构图与动作因果 | 7 个竖屏构图模板：门缝火光、手部道具首秒可读、骨钟震动、巨兽角影、虫蜡针近景、断阶抓手、雪坡逃亡；每项含前中后景比例和首秒可读面积 |
...
5. `art/series-thread-plan.json` 已拆分为三势力徽章旗帜基础母版、风格、服装、角色、地点、道具/符号的 8 个依赖批次；B00、B07、B06 已完成，其余批次仍待单独派发。
...
10. B07_STYLE 已补齐 `style_reference_data`，包含具体 HEX 色板、材质参数、地域色彩路线、势力符号派生规则、9:16 构图模板和低魔光效 token。
11. B06_COSTUMES 已生成 K001-K006 六张服装系统板，canonical PNG 均为 2160x3840、9:16、无 alpha，并同步更新资产索引与线程结果。K004 已修正为黑日白翼肃明/清明院徽记，不再漂移为虫甲胸牌；K006 保持完整旧衣结构；K005 作为北境粗粝材质层级板通过，但后续角色卡继承时必须保持可穿戴裁片、包边和缝线，不能放大为乞丐破布。
...
15. B01_CORE_CHARACTERS 已局部生成 C001-C007、C023-C026：canonical PNG 均为 2160x3840、9:16、无 alpha；C001 以用户提供的沈维桑旧参考板硬锁脸型和发型，C023-C026 已按反馈重生，明确区分逃难流民、边墙军户、普通平民和墙下集市交易人群的服饰系统、色彩、工具和社会功能。
16. B02_NORTHERN_AND_FACTION_TEMPLATES 已局部生成 C017-C021：canonical PNG 均为 2160x3840、9:16、无 alpha；C017-C019 已按反馈重生，奴兵为虫族帝国最低层炮灰兵而非被锁链奴隶，头皮虫族异化与脸部融合，脑后为细长虫须状金钱马尾辫，虫族兵徽章统一继承 P017 肃明/虫族帝国黑日白翅徽章语言。
...
|   4 | B01_CORE_CHARACTERS                | 核心、追捕线、沈家旧档、普通平民、逃难流民、边墙军户与墙下集市群像角色 | B00_FACTION_EMBLEM_FLAGS、B07_STYLE、B06_COSTUMES                                           | partial_complete_remaining_c008_retry_needed |
|   5 | B02_NORTHERN_AND_FACTION_TEMPLATES | 北境具名角色与族群/肃明层级模板                    | B00_FACTION_EMBLEM_FLAGS、B07_STYLE、B06_COSTUMES                                           | partial_complete_remaining_c016_retry_needed |
