...
已生成并返工确认 `blockout.blend`、`top-view.png`、`camera-map.png`、四张 `shot-guides/*_camera.png`、四张 `depth/*_depth.png` 和四张 `lineart/*_lineart.png`。所有 PNG 为 1280x720。
...
用户要求的步骤 1、2、3、4、5、6 已完成。用户对顶视图不可读的反馈已处理，已重新导出可读版 `top-view.png` 和 `camera-map.png`。步骤 7 已把控制图交给本地 ComfyUI API 预检并尝试执行，但因模型缺失失败；步骤 8 等待生成候选图后再由 Krita/GIMP 执行。
