...
| scene-image-resource-agent / scene master handoff | 91 | 90 | 2 | 场景母版通过，关键帧仍阻塞 |
...
用户要求的步骤 1、2、3、4、5、6 已完成。用户对顶视图不可读的反馈已处理，已重新导出可读版 `top-view.png` 和 `camera-map.png`。用户要求升级“带统一材质/贴图的场景母版”已处理，已从同一 Blender 场景导出材质锁、正反母版、关键道具关系图和调度母版图。步骤 7 已把控制图交给本地 ComfyUI API 预检并尝试执行，但因模型缺失失败；步骤 8 等待生成候选图后再由 Krita/GIMP 执行。
