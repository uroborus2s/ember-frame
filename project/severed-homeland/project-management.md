...
人和 AI 第一次进入项目，先读 `index.md`。该入口集中说明输入文档结构、五层项目记忆、中间版本生命周期、人机职责、从创意到成片的完整流程和本仓库可用工具；进入专业生产后，再按下方索引读取本文件的相关小节。

当前进度只看 `.project/status.md`，当前阻塞只看 `.project/blockers.md`，不得从聊天记录、旧预览或隐藏版本库猜测当前状态。
