...
- 对所有最终面向画面的场景图，必须把 `scene_clarity_profile` 写入 `model_visible_prompt` 和 `copy_ready`。正向提示词必须说明主光源、3-5 个大明暗关系、1-3 个留白式缓冲区、1-2 个最高细节焦点、边缘细节区域、远景 / 背景 / 重复纹理压柔方式。负向提示词必须按需排除噪声微细节、AI 斑点、脏纹理、脏灰暗部、全画面超细节、过度锐化颗粒、JPEG 伪影、混乱重复纹理、烟雾冒充细节、每个表面都雕花和低对比雾霾吞掉主体。
