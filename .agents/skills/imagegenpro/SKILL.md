...
## Center-anchor radial orientation grids

For any location 9-grid, orientation-grid, scene-consistency grid, or compass
view task, do not treat the deliverable as nine independent attractive scene
views. The main task must first define a `center_anchor_id`, such as a well,
gate, altar, table, doorway, bridge, staircase landing, or other fixed spatial
object.

The eight compass cells must orbit that same center anchor:

```text
NW camera -> looks SE at center_anchor
N camera  -> looks S  at center_anchor
NE camera -> looks SW at center_anchor
W camera  -> looks E  at center_anchor
C cell    -> approved mother/neutral center view
E camera  -> looks W  at center_anchor
SW camera -> looks NE at center_anchor
S camera  -> looks N  at center_anchor
SE camera -> looks NW at center_anchor
```

Before image generation, prepare a compact consistency package:

```text
center_anchor_id and coordinate
adjacent fixed anchors and their side relative to center_anchor
road/path topology relative to center_anchor
anchor orbit camera map or accepted radial blockout grid
approved style/mother plate
explicit rejection examples from prior drafts, if any
```

Generation prompts must state that layout/control evidence has higher priority
than beauty. Project-specific constraints for fixed props, buildings, paths,
doorways, secondary anchors, and rejected prior-draft errors must be copied from
the current task's control package into the image prompt. Do not encode those
case-specific constraints in this reusable skill.

QC must reject a result when the center anchor changes, an adjacent fixed object
changes side, fixed furniture moves into a different functional zone, an
adjacent secondary anchor disappears or becomes a different structure, or the
scene path/circulation topology changes between cells. In that case do not
promote the candidate; rebuild the control package or generate from a stronger
blockout.
